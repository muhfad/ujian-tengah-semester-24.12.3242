    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden flex flex-col hover:shadow-md transition duration-300">
            ...
            <div class="p-6 flex flex-col flex-grow">
                ...
                <p class="text-slate-500 text-xs mb-4 font-semibold">
                    📍 {{ $event->location }} • 📅 {{ \Carbon\Carbon::parse($event->date)->format('d M Y') }}
                </p>
                <p class="text-slate-600 text-sm line-clamp-2 mb-5 font-normal">
                    {{ $event->description ?? 'Tidak ada rincian deskripsi tambahan untuk acara ini.' }}
                </p>
                
                <div class="mt-auto flex justify-between items-center pt-4 border-t border-slate-100">
                    <div>
                        <p class="text-xs text-slate-400 font-bold uppercase tracking-wider">Harga Tiket</p>
                        <p class="text-lg font-extrabold text-indigo-600">
                            {{ $event->price == 0 ? 'Gratis' : 'Rp ' . number_format($event->price, 0, ',', '.') }}
                        </p>
                    </div>
                    <a href="{{ route('events.show', $event->id) }}" class="bg-slate-900 hover:bg-indigo-600 text-white font-bold py-2 px-4 rounded-xl transition text-xs shadow-sm shadow-slate-100">
                        Beli Tiket
                    </a>
                </div>
            </div>
